...
class _SignInState extends State<SignIn> {
  ...
  Widget _buildForm(BuildContext context, LoginFormState state) {
    ...
    return BlocBuilder<LoginCubit, LoginFormState>(
        builder: (context, state) => Container(
          ...
          child: MaterialButtonModel(highlightColor: Colors.transparent, splashColor: CustomTheme.loginGradientEnd,
            ...
            onPressed: () => _signinButtonOnPressed(context), padding: const EdgeInsets.symmetric(vertical: 10.0, horizontal: 42.0),
          ),
        )
    );
  }
  ...
  void _signinButtonOnPressed(BuildContext context) {
    final loginCubit = context.read<LoginCubit>();
    loginCubit.onSubmit();
    bool isValid = loginCubit.state.isValid;
    if(!isValid) {
